...
class Summarize:
    ...
    def summarizeDets(self):
        ...
        stats[3] = self.summarize(1, iouThr=.75, maxDets=self.params.maxDets[2])
        stats[4] = self.summarize(1, iouThr=.80, maxDets=self.params.maxDets[2])
        stats[5] = self.summarize(1, iouThr=.90, maxDets=self.params.maxDets[2])
        ...
        stats[11] = self.summarize(0, iouThr=.75, maxDets=self.params.maxDets[2])
        stats[12] = self.summarize(0, iouThr=.80, maxDets=self.params.maxDets[2])
        stats[13] = self.summarize(0, iouThr=.90, maxDets=self.params.maxDets[2])
