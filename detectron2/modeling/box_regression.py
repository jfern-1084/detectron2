...
def _dense_box_regression_loss(
    ...
    pred_proposals : List[torch.Tensor],
    ...
    if box_reg_loss_type == "smooth_l1":
        ...
        loss_box_reg = compute_diou(
            cat(pred_anchor_deltas, dim=1)[fg_mask],
            gt_anchor_deltas[fg_mask],
            box2box_transform.weights,
            box2box_transform.scale_clamp
        )

    elif box_reg_loss_type == "diou_mmdet":
        pred_proposals = cat(pred_proposals, dim=1)
        pred_proposals = pred_proposals.view(-1, pred_proposals.shape[-1])
        pos_mask = fg_mask.view(-1)
        loss_box_reg = compute_diou_mmdet(
            pred_proposals[fg_mask], cat(gt_boxes)[fg_mask]
        )
    elif box_reg_loss_type == "ciou_mmdet":
        pred_proposals = cat(pred_proposals, dim=1)
        pred_proposals = pred_proposals.view(-1, pred_proposals.shape[-1])
        pos_mask = fg_mask.view(-1)
